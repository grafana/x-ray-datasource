 ...
 * alternatives.
 */
export function QueryEditor(props: Omit<XrayQueryEditorFormProps, 'groups' | 'regions' | 'accountIds'>) {
  ...
  const accountIds = useAccountIds(props.datasource, props.range, props.query.group);
  ...
  if (!(groups && regions && accountIds)) {
    return <Spinner />;
  } else {
    return <QueryEditorForm {...{ ...props, groups, regions, accountIds }} />;
  }
}
